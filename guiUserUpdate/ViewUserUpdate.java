...
public class ViewUserUpdate {
	...
	private static boolean invalidEmail(String email) {
	    if (email == null || email.isBlank()) {
	        ...
	        a.setTitle("Invalid Email Address");
	        a.setHeaderText("Email not updated");
	        a.setContentText("Email address cannot be empty.");
	        a.showAndWait();
	        return true;
	    }
	    if (!EmailValidator.isValid(email)) {
	        String msg = EmailValidator.error(email);
	        if (msg == null || msg.isBlank()) msg = "Please enter a valid email address.";
	        var a = new Alert(Alert.AlertType.ERROR);
	        a.setTitle("Invalid Email Address");
	        a.setHeaderText("Email not updated");
	...
	private ViewUserUpdate() {
        ...
        button_UpdateEmailAddress.setOnAction((event) -> {
            // prefill dialog with current email (nice UX)
            dialogUpdateEmailAddresss.getEditor().setText(
                (label_CurrentEmailAddress.getText() == null ||
                 label_CurrentEmailAddress.getText().equals("<none>")) ? "" : label_CurrentEmailAddress.getText()
            );

            result = dialogUpdateEmailAddresss.showAndWait();
            result.ifPresent(newEmail -> {
                // 1) validate
                if (invalidEmail(newEmail)) return;
                ...
                // 3) refresh cache + UI
                theDatabase.getUserAccountDetails(theUser.getUserName());
                String updated = theDatabase.getCurrentEmailAddress();
                theUser.setEmailAddress(updated);
                label_CurrentEmailAddress.setText(
                    (updated == null || updated.isEmpty()) ? "<none>" : updated
                );

                // 4) optional success toast
                var ok = new Alert(Alert.AlertType.INFORMATION);
                ok.setTitle("Email Updated");
                ok.setHeaderText(null);
                ok.setContentText("Email address updated successfully.");
                ok.showAndWait();
            });
        });
